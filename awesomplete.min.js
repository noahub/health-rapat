...
(() => {
			...
			b.length >= this.minChars && 0 < this._list.length
				? ((this.index = -1),
					...
					(this.suggestions = this._list
						.map((d) => new h(this.data(d, b)))
						.filter((d) => this.filter(d, b))
						.sort(this.sort)
						.slice(0, this.maxItems)),
					this.suggestions.forEach((d) => {
						this.ul.appendChild(this.item(d, b));
					}),
	...
	e.all = [];
	e.FILTER_CONTAINS = (a, b) => RegExp(c.regExpEscape(b.trim()), "i").test(a);
	e.FILTER_STARTSWITH = (a, b) =>
		RegExp("^" + c.regExpEscape(b.trim()), "i").test(a);
	e.SORT_BYLENGTH = (a, b) =>
